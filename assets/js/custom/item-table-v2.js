class ItemTablev2 {
  ...
  initializeEvents() {
    ...
    if (this.options.showMoreTemplate != null) {
      this.showMoreDetailsEvent()
    }
  ...
  showMoreDetailsEvent(){
    ...
    $(this.options.container).on('click', '.show-more', function () {
      ...
      if (row.child.isShown()) {
        ...
        console.log($(this).closest('tr').data('rowData'))
        row.child(self.options.showMoreTemplate({"d": $(this).closest('tr').data('rowData')})).show();
        tr.addClass('shown');
      }
    });
  }
  ...
  populateTable(finalValueArray) {
    var self = this;    
    console.log(self.options.tableTemplate);
    let $table = $(self.options.tableTemplate({ primaryPropertiesInOrder: self.options.primaryPropertiesInOrder }));
    for (var i = 0; i < finalValueArray.length; i++) {
      let row = finalValueArray[i];
      let $row = $(self.options.rowTemplate({ row: row.values, otherDetails: row.otherDetails, value: row.value }));
