class MaintenanceTasks {
  ...
  loadResources() {
    ...
    $.ajax({
      method: 'GET',
      url: _config.api2().maintenanceTasks,
      headers: {
      ...
      success: function (result) {
        if (result.success == true) {
          ...
          Utilities.showLoader('We are analysing your infrastructure for maintenance tasks. Please check back after some time.');
    ...
    let itemTable = new ItemTable({
      ...
      onAction: function (target, rowData) {
        ...
        function (result) {
          ...
          $.ajax({
            method: 'POST',
            url: _config.api2().maintenanceTaskFix,
            contentType: 'application/json',
            headers: {
              Authorization: self.router.token,
            },
...
$(function onDocReady() {
  let maintenanceTasks = new MaintenanceTasks()
});
